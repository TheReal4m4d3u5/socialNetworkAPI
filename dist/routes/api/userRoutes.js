...
import Thought from '../../models/thought.js'; // Assuming a Thought model exists
const router = Router();
...
router.post('/:userId/friends/:friendId', async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate(req.params.userId, { $addToSet: { friends: req.params.friendId } }, // Add friend to friends array
        { new: true });
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
...
router.delete('/:userId/friends/:friendId', async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate(req.params.userId, { $pull: { friends: req.params.friendId } }, // Remove friend from friends array
        { new: true });
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
